...
class TestFunc(unittest.TestCase):
    ...
    def test_argument_variations_of_Func_usage(
        self, _, target_member, write_pattern, argument_mapping, return_mapping
    ):
    ...
    def test_argument_invalid_return_mapping_type_returns_TypeError(self):
        ...
        with self.assertRaises(TypeError):
            Func(
                self.target_member,
                self.write_pattern,
                self.incorrect_argument_mapping,
                invalid_return_mapping,
            )
    ...
    def test_func_returns_correct_value_for_return_argument_mapping(self):
        self.assertEqual(
            Func(
                ...
                self.return_mapping,
            ).process_request(self.process_request_value),
            7,
        )
    ...
    def test_process_request(self):
        self.assertTrue(
            Func(
                ...
                self.argument_mapping,
                self.return_mapping,
            ).can_process(self.process_request_value)
        )
